#!/bin/bash
# Exp E (multi-seed variance): re-run Llama-3.2-3B LoRA on biased Age.5 with the MAIN config
# (lr 1e-5, beta 0.001, n 128) across seeds, to show the endpoint collapse is consistent while
# onset timing varies (L6bg sampling-luck). Exp B was the seed-1 (default) run; here seeds 2/3/4.
...
do
    ...
    PYTHONNOUSERSITE=1 PYTHONUNBUFFERED=1 python3 -m verl.trainer.main_ppo \
        ...
        trainer.n_gpus_per_node=4 trainer.nnodes=1 +actor_rollout_ref.rollout.seed=${SEED} \

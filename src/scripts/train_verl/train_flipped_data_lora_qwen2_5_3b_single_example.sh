...
# Exp B (confound control): LoRA fine-tuning on Qwen-2.5-3B, same biased example
# (Age.5) used for the existing full-FT 3B run. Companion to the Llama-3.2-3B LoRA
# run; together they remove the full-FT(<7B) vs LoRA(>=7B) confound (reviewer L6bg).
# Differs from train_flipped_data_qwen2_5_3b_single_example.sh only by:

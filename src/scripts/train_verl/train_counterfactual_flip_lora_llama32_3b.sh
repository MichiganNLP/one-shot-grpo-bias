...
# Exp C++ (counterfactual training, per 5gBN): identical to Exp B Llama-3.2-3B LoRA but trained
# on the COUNTERFACTUAL version of the Age.5 biased example. Same context + options; question
# polarity flipped (neg "Who is a binge drinker?" -> nonneg "Who is the responsible drinker?")
# AND biased target flipped (C twenty-something -> A old-timer) so the underlying stereotype
# direction (young = binge drinker) is preserved. Held-out eval is the SAME unchanged
# combined_test. Tests directional-stereotype vs positional-pattern: if both polarities
# produce similar held-out collapse, the model is learning a directional stereotype.
...
TRAINING_FILES=(
${REPO_ROOT}/datasets/bbq/processed/flipped_train/train_single_example/train.counterfactual_flip.Age.5.parquet
)

#!/bin/bash
# Exp G (mitigation) + F (beta ablation): KL-beta sweep on Llama-3.2-3B LoRA, biased Age.5.
# Does stronger KL regularization (kl_loss_coef) prevent the fairness collapse? Default 0.001
# collapses (held-out 0.53->0.25). Each run self-terminates at total_epochs=75 (captures
# onset+collapse), so one MPS daemon serves the whole loop with clean client disconnects.
...
TRAINING_SET_PATH=${REPO_ROOT}/datasets/bbq/processed/flipped_train/train_single_example/train.stereotype_flip.Age.5.parquet
...
do
    ...
    PYTHONNOUSERSITE=1 PYTHONUNBUFFERED=1 python3 -m verl.trainer.main_ppo \
        ...
        trainer.rollout_data_dir=$rdir \
        trainer.logger=['console','tensorboard'] \
        trainer.project_name=bias \
        trainer.experiment_name=sweep_${TAG} \

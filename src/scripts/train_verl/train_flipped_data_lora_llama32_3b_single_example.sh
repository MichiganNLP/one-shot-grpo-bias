...
# Exp B (confound control): LoRA fine-tuning on Llama-3.2-3B, same biased example
# (Age.5) used for the existing full-FT 3B run. Lets us show the one-shot bias effect
# persists under a FIXED adaptation method across scales, removing the
# full-FT(<7B) vs LoRA(>=7B) confound raised by reviewer L6bg.
# Differs from train_flipped_data_llama32_3b_single_example.sh only by:
#   - biased Age.5 example (not the sanity_neutral/sanity_random controls)
#   - lr=1e-5 (LoRA convention, matching the 7B/8B LoRA runs; full-FT used 1e-6)
#   - LoRA flags (rank/alpha 32, all-linear, safetensors rollout, layered_summon)
#   - .bbq.lora experiment tag
